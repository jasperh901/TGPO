...
def _debug_print(*args, **kwargs):
    if os.getenv("RLLA_REWARD_DEBUG", "0") == "1":
        print(*args, **kwargs)
...
def match_score(list1, list2):
    ...
    if os.getenv("REFINEDREWARD", 0) == "1":
        _debug_print("REFINEDREWARD is set to 1, so strict match is used")
        if list1 != list2:
            return 0.0
...
def customize_format_reward_func(completions, answer, step, max_possible_reward, min_possible_reward, **kwargs):
    if str(os.getenv("MAX1STEP30MAX3", 0)) == "1":
        _debug_print("MAX1STEP30MAX3 is set to 1, so max 1 -> 30 steps -> max 3")
        if step >= 30:
    ...
    if str(os.getenv("SCHEDULEREWARD", 0)) == "1":
        _debug_print("SCHEDULEREWARD is set to 1, so schedule reward is used")
    ...
    _debug_print("\n======= Answer ======= ")
    _debug_print(answer[0])
    _debug_print("\n======= Responses ======= ")
    for idx, response in enumerate(responses):
        _debug_print(f"*** Response {idx+1}***\n{response}")
    ...
    _debug_print("\n======= Reward for <format> =======")
    _debug_print("Reward function for <format> is called ...")
    _debug_print(rewards)
    return rewards
...
def customize_length_reward_func(completions, answer, step, max_possible_reward, min_possible_reward, **kwargs):
    ...
    if os.getenv("SCHEDULELENGTH", 0) == "1":
        _debug_print("SCHEDULELENGTH is set to 1, so schedule max reward for length is used")
    ...
    _debug_print("\n======= Reward for <length> =======")
    _debug_print("Reward function for <length> is called ...")
    _debug_print(rewards)
    return rewards
...
def compute_tool_call_reward(gt_tools, pd_tools, max_possible_reward, min_possible_reward):
    if gt_tools == pd_tools:
        _debug_print("Max possible score:", "Exact Match!")
        _debug_print("Score:", max_possible_reward)
        return max_possible_reward
    
    if os.getenv("COARSEREWARD", 0) == "1":
        _debug_print("COARSEREWARD is set to 1, so coarse reward is used")
        if gt_tools != pd_tools:
    ...
    for gt_tool in gt_tools:
        ...
        if str(os.getenv("INTERMEDIATEREWARD", 0)) == "1":
            _debug_print("INTERMEDIATEREWARD is set to 1, so local max possible is changed")
            local_max_possible += 1.0
        else:
    ...
    _debug_print()
    _debug_print("Max possible score:", local_max_possible)
    _debug_print("Score:", score)
...
def customize_correctness_reward_tool(completions, answer, step, max_possible_reward, min_possible_reward, **kwargs):
    ...
    _debug_print("Reward function for <tool call> correctness is called ...")
    _debug_print(rewards)
    return rewards
...
def compute_score(solution_str, ground_truth, step=0):
    ...
    if str(os.getenv("CORRECTMAX1", 0)) == "1":
        _debug_print("CORRECTMAX1 is set to 1, so max score is set to 1")
    ...
    if str(os.getenv("WITHLENGTH", 0)) == "1":
        _debug_print("WITHLENGTH is set to 1, so length score is set!")
