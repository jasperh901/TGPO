...
class vLLMRollout(BaseRollout):
    ...
    def __init__(self, actor_module: nn.Module, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        engine_overrides = {}
        if config.get('num_gpu_blocks_override') is not None:
            engine_overrides['num_gpu_blocks_override'] = int(config.num_gpu_blocks_override)
        ...
        kwargs = dict(
            n=1,
            logprobs=None,  # behavior-policy log probabilities are recomputed after filtering
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        sampling_seed = prompts.meta_info.get('sampling_seed')
        if sampling_seed is not None:
            kwargs['seed'] = int(sampling_seed)
        if not do_sample:
            kwargs = {
                'best_of': 1,
                'top_p': 1.0,
