...
class ActorRolloutRefWorker(Worker):
    ...
    def init_model(self):
        ...
        if self._is_rollout:
            ...
            if runtime_state is not None:
                ...
                del runtime_state
                from verl.utils.complete_checkpoint import release_cpu_allocator
    ...
    def save_checkpoint(self, local_path, hdfs_path=None, complete_state=False):
        assert self._is_actor
        import torch
        from verl.utils.complete_checkpoint import (
            save_rank_states_serially, release_cpu_allocator,
            checkpoint_memory_event, flush_checkpoint_file,
        )
        # Serialize and release rank-local copies before collecting the full model.
        save_rank_states_serially(self, local_path, complete_state)
        if self._is_offload_param:
            load_fsdp_param_and_grad(module=self.actor_module_fsdp,
                                     device_id=torch.cuda.current_device(),
                                     load_grad=self._is_offload_grad)
        ...
        checkpoint_memory_event(self, 'before_full_model_gather')
        ...
        checkpoint_memory_event(self, 'after_full_model_gather')
        error = [None]
        if self.rank == 0:
            try:
                print(f'Saving actor checkpoint to {local_path}')
                # Smaller files bound serializer scratch space; values/dtypes stay exact.
                self.actor_module.save_pretrained(local_path, state_dict=state_dict,
                                                 max_shard_size='1GB')
                self.tokenizer.save_pretrained(local_path)
                from pathlib import Path
                for path in Path(local_path).iterdir():
                    if path.is_file() and not path.name.startswith(('runtime_rank_', 'optimizer_rank_')):
        ...
        if error[0] is not None:
            raise RuntimeError(f'Checkpoint model save failed: {error[0]}')
        if self._is_offload_param:
            offload_fsdp_param_and_grad(module=self.actor_module_fsdp, offload_grad=self._is_offload_grad)
        release_cpu_allocator()
        checkpoint_memory_event(self, 'checkpoint_worker_complete')
    ...
    def _load_checkpoint_impl(self, local_path):
        ...
        # Hugging Face omits the alias when saving tied Qwen embeddings.
        if self.actor_module.config.tie_word_embeddings and 'lm_head.weight' not in state_dict:
        ...
        checkpoint = torch.load(optimizer_file, map_location='cpu', weights_only=False, mmap=True)
