...
class RayPPOTrainer(object):
    ...
    def init_workers(self):
        ...
        from verl.utils.complete_checkpoint import validate_complete_checkpoint_config
        validate_complete_checkpoint_config(
            self.config, use_critic=self.config.algorithm.adv_estimator == 'gae')
    ...
    def _save_checkpoint(self):
        if self.config.trainer.get('complete_checkpoint', False):
            from verl.utils.complete_checkpoint import (
                capture_rng, commit_snapshot, config_contract, validate_complete_checkpoint_config)
            validate_complete_checkpoint_config(self.config, use_critic=self.use_critic)
    ...
    def fit(self):
        ...
        if resume_step and self.config.trainer.get('complete_checkpoint', False):
            ...
            from verl.utils.complete_checkpoint import validate_resume_provenance
            validate_resume_provenance(saved)
