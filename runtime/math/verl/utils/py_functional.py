...
try:
    ...
    class Metric:  # Compatibility with the pre-metric verl release used by this project.
        pass


# Forking two timeout workers concurrently can race with libraries that
# register descriptor-ownership hooks. Only process creation is serialized;
# the isolated scoring functions still execute in parallel.
_MP_PROCESS_START_LOCK = threading.Lock()


class TimeoutWorkerError(RuntimeError):
    """The isolated timeout worker failed independently of the target function."""
    ...
    def decorator(func):
        if use_signals:
            ...
            def wrapper_mp(*args, **kwargs):
                ...
                try:
                    try:
                        with _MP_PROCESS_START_LOCK:
                            process.start()
                        started = True
                    except Exception as error:
                        raise TimeoutWorkerError(
                            f"Failed to start timeout worker for {func.__name__}: "
                    ...
                    if process.is_alive():
                        ...
                        if process.is_alive():
                            # SymPy can remain inside native code after SIGTERM. A
                            # live timeout worker must never leak into subsequent
                            # reward batches, where thousands of such workers can
                            # eventually exhaust the host process table.
                            process.kill()
                            process.join(timeout=1.0)
                        if process.is_alive():
                            raise TimeoutWorkerError(
                                f"Timed-out process {process.pid} could not be reaped after SIGKILL."
                            )
                        raise TimeoutError(
                    ...
                    try:
                        ...
                        if exitcode is not None and exitcode != 0:
                            raise TimeoutWorkerError(
                                f"Child process exited with error (exitcode: {exitcode}) "
                                f"before returning result."
                            ) from err
                        raise TimeoutWorkerError(
                            f"Child process finished without returning a result "
                            f"(exitcode: {exitcode})."
                        ) from err
                finally:
                    q.close()
                    ...
                    if started:
                        try:
                            ...
                            # Preserve the stronger unreaped-process error above.
                            pass
