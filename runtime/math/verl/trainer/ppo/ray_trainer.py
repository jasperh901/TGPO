...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1,
    ...
    if adv_estimator == 'gae':
        ...
        data.batch['returns'] = returns
            ...
            'scppo_sweep_shadow'):
        reward_components = [
            data.batch['token_level_scores_correctness'],
            data.batch['token_level_scores_length'],
        ]
        index = data.non_tensor_batch['uid']
        responses = data.batch['responses']
        response_length = responses.size(-1)
...
def prepare_scppo_calibration_batch(data: DataProto, scppo_config,
    ...
    objective_advantages, statistics = core_algos.compute_scppo_calibration_inputs(
        reward_components=[
            calibration.batch['token_level_scores_correctness'],
            calibration.batch['token_level_scores_length'],
        ],
        eos_mask=response_mask,
...
def attach_moco_cagrad_full_batch(data: DataProto, scppo_config) -> dict:
    """Attach every objective advantage without changing accepted-batch order."""
    ...
    for uid in data.non_tensor_batch['uid']:
        ...
        digest.update(b'\n')
    for key in (
            'responses', 'token_level_scores_correctness',
...
class RayPPOTrainer(object):
    ...
    def _save_checkpoint(self):
        if self.config.trainer.get('complete_checkpoint', False):
            ...
            moco = self.config.actor_rollout_ref.actor.get('moco_cagrad_shadow', {})
            exact_moco = (self.config.algorithm.adv_estimator == 'scppo_shadow'
                          and moco.get('enabled', False) and moco.get('apply_update', False)
                          and not self.config.algorithm.scppo.calibration.enabled)
            if self.use_critic or (self.config.algorithm.adv_estimator not in ('gdpo', 'dvao')
                                   and not exact_moco):
                raise ValueError('Complete checkpoints support GDPO/DVAO and exact MoCo math only')
        ...
        # Resource-constrained runs keep a single recoverable model snapshot.
        # The trainer writes a complete step directory first, then removes only
        # older completed step directories so an interrupted save cannot delete
        ...
        if keep_latest > 0:
            ...
            metadata_path = os.path.join(
                self.config.trainer.default_local_dir, 'latest_checkpoint.json'
            )
    ...
    def _audit_resume_step(self, batch):
        from verl.utils.complete_checkpoint import atomic_json, tensor_digest
        audit = {
            'step': self.global_steps,
            'batch': {key: tensor_digest(batch.batch[key]) for key in (
                'input_ids', 'responses', 'attention_mask', 'token_level_rewards', 'advantages')},
            'workers': self.actor_rollout_wg.checkpoint_audit_digest(),
        }
        if 'moco_objective_advantages' in batch.batch:
            audit['batch']['moco_objective_advantages'] = tensor_digest(
                batch.batch['moco_objective_advantages'])
        reference = self.config.trainer.get('resume_audit_reference')
        path = os.path.join(self.config.trainer.default_local_dir,
                            f'audit_step_{self.global_steps}_{"resumed" if reference else "continuous"}.json')
        atomic_json(path, audit)
        if reference:
            with open(reference) as stream:
                expected = json.load(stream)
            if audit != expected:
                raise RuntimeError(f'RESUME_AUDIT_FAILED: {path} differs from {reference}')
            print(f'RESUME_AUDIT_PASSED step={self.global_steps} exact_tensor_hashes=true', flush=True)
    ...
    def fit(self):
        ...
        if resume_step < 0 or resume_step > self.total_training_steps:
            raise ValueError(
                f'resume_step must be in [0, {self.total_training_steps}], got {resume_step}'
            )
        ...
        if resume_step and self.config.trainer.get('complete_checkpoint', False):
            ...
            from moco_hparam_probe import probe_spec, validate_driver_fork
            probe = probe_spec(self.config.actor_rollout_ref.actor)
            is_probe_source = (probe is not None and
                               os.path.realpath(resume_path) ==
                               os.path.realpath(probe['source_checkpoint']))
            if is_probe_source:
                validate_driver_fork(self.config, saved, resume_path, config_contract)
            elif saved['config_contract'] != config_contract(self.config):
            ...
            for field, env in [('runtime_sha256', 'TRAIN_RUNTIME_SHA256'),
                ...
                if field == 'runtime_sha256' and is_probe_source:
                    continue  # Explicit source provenance was validated above.
                ...
                with _timer('step', timing_raw):
                    with _timer('gen', timing_raw):
                        ...
                        while accepted_groups < target_groups:
                            ...
                            with _timer('rollout_only', timing_raw):
                            ...
                            with _timer('reward_only', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        if (self.config.algorithm.adv_estimator == 'scppo_lagged' and
                            ...
                            gdpo_advantages, _ = core_algos.compute_gdpo_outcome_advantage(
                                reward_components=[
                                    batch.batch['token_level_scores_correctness'],
                                    batch.batch['token_level_scores_length'],
                                ],
                                eos_mask=response_mask,
                    ...
                    if self.global_steps == int(self.config.trainer.get('resume_audit_step', -1)):
                        self._audit_resume_step(batch)
