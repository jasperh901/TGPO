...
_REWARD_COMPONENT_NAME = re.compile(r'^[A-Za-z][A-Za-z0-9_]*$')


def attach_reward_output(data: DataProto, reward_output):
    """Validate a reward-manager result and attach its objective tensors."""
    if not isinstance(reward_output, (tuple, list)):
        raise TypeError('reward manager must return a tuple or list')
    if len(reward_output) == 2 and isinstance(reward_output[1], dict):
        reward_tensor, component_tensors = reward_output
    elif len(reward_output) == 4:
        reward_tensor, format_tensor, correctness_tensor, length_tensor = reward_output
        component_tensors = {
            'format': format_tensor,
            'correctness': correctness_tensor,
            'length': length_tensor,
        }
    else:
        raise ValueError(
            'reward manager must return (total, components) or the legacy '
            '(total, format, correctness, length) tuple'
        )

    expected_shape = data.batch['responses'].shape

    def validate_tensor(label, tensor):
        if not isinstance(tensor, torch.Tensor):
            raise TypeError(f'{label} must be a torch.Tensor')
        if tensor.shape != expected_shape:
            raise ValueError(
                f'{label} shape {tuple(tensor.shape)} does not match responses '
    ...
    validate_tensor('total reward', reward_tensor)
    if not component_tensors:
        raise ValueError('reward manager returned no objective components')
    for name, tensor in component_tensors.items():
        if not isinstance(name, str) or _REWARD_COMPONENT_NAME.fullmatch(name) is None:
            raise ValueError(f'invalid reward component name: {name!r}')
        validate_tensor(f'reward component {name!r}', tensor)
...
def compute_data_metrics(batch, use_critic=True):
    ...
    component_prefix = 'token_level_scores_'
    sequence_component_scores = {
        key.removeprefix(component_prefix): batch.batch[key].sum(-1)
        for key in sorted(batch.batch.keys())
        if isinstance(key, str) and key.startswith(component_prefix)
    }
    ...
    for name, values in sequence_component_scores.items():
        metrics.update({
            f'critic/{name}_score/mean': torch.mean(values).detach().item(),
            f'critic/{name}_score/max': torch.max(values).detach().item(),
            f'critic/{name}_score/min': torch.min(values).detach().item(),
        })
    return metrics
...
class RayPPOTrainer(object):
    ...
    def _save_checkpoint(self):
        ...
        # Driver state is small and completes the worker-side model/optimizer
        # snapshot, allowing a resumed run to continue from the same step.
        state_dir = os.path.join(actor_local_path, 'training_state')
        ...
        }, os.path.join(state_dir, 'driver_state.pt'))
        with open(os.path.join(state_dir, 'DRIVER_COMPLETE'), 'w', encoding='utf-8') as marker:
            marker.write('driver state complete\n')

        keep_last = max(1, int(self.config.trainer.get('checkpoint_keep_last_n', 1)))
        actor_root = os.path.join(self.config.trainer.default_local_dir, 'actor')
        candidates = []
        for name in os.listdir(actor_root):
            if name.startswith('global_step_'):
                try:
                    candidates.append((int(name.rsplit('_', 1)[1]), name))
                except ValueError:
                    continue
        for _, name in sorted(candidates)[:-keep_last]:
            shutil.rmtree(os.path.join(actor_root, name), ignore_errors=True)
    ...
    def fit(self):
        ...
        resume_step = int(self.config.trainer.get('resume_step', 0) or 0)
        if resume_path:
            driver_state_path = os.path.join(str(resume_path), 'training_state', 'driver_state.pt')
            if not os.path.isfile(driver_state_path):
                raise RuntimeError(f'missing driver state: {driver_state_path}')
            driver_state = torch.load(driver_state_path, map_location='cpu', weights_only=False)
            resume_step = int(driver_state['global_steps'])
            random.setstate(driver_state['python'])
            np.random.set_state(driver_state['numpy'])
            torch.set_rng_state(driver_state['torch'])
            if driver_state.get('cuda') is not None and torch.cuda.is_available():
                torch.cuda.set_rng_state_all(driver_state['cuda'])
            print(f'Resuming PPO from completed step {resume_step}', flush=True)
                    ...
                    with _timer('gen', timing_raw):
                        ...
                        while accepted_groups < target_groups:
                            ...
                            generated_response_tokens += int(
                                generated.batch['attention_mask'][:, -response_width:].sum().item()
                            )
                            ...
                            reward_tensor, _ = attach_reward_output(
                                candidate,
                                self.reward_fn(candidate, self.global_steps),
                            )
                        ...
                        metrics.update({
                            ...
                            'resource/generated_response_tokens': generated_response_tokens,
                        })
