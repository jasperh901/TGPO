...
def hard_cap_partitions(lengths, capacity, target_count=None):
    """First-fit-decreasing with a hard token bound and no dropped samples.

    A sum/capacity estimate followed by unconstrained balancing can put two
    long sequences in one over-budget bin. Split, never merge, when matching
    the collective-safe micro-batch count selected across FSDP ranks.
    """
    if not lengths or capacity <= 0 or any(n <= 0 or n > capacity for n in lengths):
        raise ValueError('Every sequence must have positive length within the token capacity')
    bins, used = [], []
    for index in sorted(range(len(lengths)), key=lambda i: (-lengths[i], i)):
        for bin_index, total in enumerate(used):
            if total + lengths[index] <= capacity:
    ...
    if target_count is not None:
        if target_count < len(bins) or target_count > len(lengths):
            raise ValueError('Invalid shared micro-batch count')
        while len(bins) < target_count:
            candidates = [i for i, partition in enumerate(bins) if len(partition) > 1]
            chosen = max(candidates, key=lambda i: sum(lengths[j] for j in bins[i]))
            bins.append([bins[chosen].pop()])
    return sorted((sorted(partition) for partition in bins), key=lambda partition: partition[0])
...
def rearrange_micro_batches(batch: TensorDict, max_token_len, dp_group=None):
    ...
    initial_partitions = hard_cap_partitions(seq_len_effective, max_token_len)
    num_micro_batches = len(initial_partitions)
    ...
    micro_bsz_idx = hard_cap_partitions(seq_len_effective, max_token_len, num_micro_batches)
