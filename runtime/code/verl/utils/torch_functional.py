...
def get_constant_schedule_with_warmup(
    ...
    def lr_lambda(current_step):
        # Zero warmup means LR=base_lr from the first optimizer update.
        # max(1, num_warmup_steps) incorrectly made update 1 a zero-LR step.
        if num_warmup_steps == 0:
            return 1.0
        return min(1.0, float(current_step) / float(num_warmup_steps))
