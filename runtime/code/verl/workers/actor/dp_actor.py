...
class DataParallelPPOActor(BasePPOActor):
    ...
    def __init__(
        ...
        if self.moco_cagrad_shadow_enabled:
            ...
            cache_budget = int(moco_config.get('fixed_weight_cache_bytes', 0))
            if cache_budget and self.moco_cagrad_apply_update:
                from functools import partial
                from moco_fsdp_cache import prepare_cached_active_update
                self.moco_cagrad_execute = partial(
                    prepare_cached_active_update, cache_budget_bytes=cache_budget
                )
