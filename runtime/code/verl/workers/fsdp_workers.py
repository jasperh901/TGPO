...
class ActorRolloutRefWorker(Worker):
    ...
    def init_model(self):
        ...
        if self._is_actor or self._is_rollout:
            ...
            self.actor_module_fsdp, self.actor_optimizer, self.actor_lr_scheduler, self.actor_model_config = self._build_model_optimizer(
                # Keep model.path as the frozen KL reference. Only the actor
                # starts from the resumed policy weights.
                model_path=self.config.actor.get('resume_from_checkpoint') or self.config.model.path,
        ...
        if self._is_actor:
            ...
            # Rollout construction seeds CUDA and captures graphs. Restore RNG
            # after initialization so it is not immediately overwritten.
            resume_path = self.config.actor.get('resume_from_checkpoint', None)
            if resume_path:
                self._load_training_state(str(resume_path))
    ...
    def _to_cpu(value):
        ...
        if isinstance(value, dict):
            return {key: ActorRolloutRefWorker._to_cpu(item) for key, item in value.items()}
        if isinstance(value, list):
            return [ActorRolloutRefWorker._to_cpu(item) for item in value]
        if isinstance(value, tuple):
            return tuple(ActorRolloutRefWorker._to_cpu(item) for item in value)
        return value

    def _load_training_state(self, checkpoint_path):
        """Restore rank-local optimizer, scheduler, and RNG state."""
        state_dir = os.path.join(checkpoint_path, 'training_state')
        optimizer_path = os.path.join(state_dir, f'optimizer_rank{self.rank}.pt')
        if not os.path.isfile(optimizer_path):
            raise RuntimeError(f'missing optimizer state for rank {self.rank}: {optimizer_path}')
        self.actor_optimizer.load_state_dict(
            torch.load(optimizer_path, map_location='cpu', weights_only=False)
        )
        scheduler_path = os.path.join(state_dir, 'scheduler.pt')
        if self.actor_lr_scheduler is not None and os.path.isfile(scheduler_path):
            self.actor_lr_scheduler.load_state_dict(
                torch.load(scheduler_path, map_location='cpu', weights_only=False)
            )
        if getattr(self.actor, 'moco_cagrad_tracker', None) is not None:
            from moco_checkpoint import restore_worker_tracker
            restore_worker_tracker(self, state_dir)
        rng_path = os.path.join(state_dir, f'rng_rank{self.rank}.pt')
        if os.path.isfile(rng_path):
            ...
            if self._is_rollout:
                if 'rollout_cuda' in rng:
                    self.rollout_sharding_manager.gen_random_states = rng['rollout_cuda']
                elif self.rank == 0:
                    print('Legacy checkpoint lacks rollout CUDA RNG; optimizer resume is supported, '
                          'but bitwise trajectory replay is not guaranteed.', flush=True)
                self.rollout_sharding_manager.torch_random_states = rng['cuda']
        torch.distributed.barrier()
        if self.rank == 0:
            print(f'Restored optimizer/scheduler/RNG state from {checkpoint_path}', flush=True)
    ...
    def save_checkpoint(self, local_path, hdfs_path=None):
        ...
        state_dir = os.path.join(local_path, 'training_state')
        if self.rank == 0:
            os.makedirs(state_dir, exist_ok=True)
        torch.distributed.barrier()
        # Optimizer state remains rank-local, matching FSDP parameter
        # ownership and avoiding a full-state gather during checkpointing.
        torch.save(
            self._to_cpu(self.actor_optimizer.state_dict()),
            os.path.join(state_dir, f'optimizer_rank{self.rank}.pt'),
        )
        torch.save(
            {'torch': torch.get_rng_state(), 'cuda': torch.cuda.get_rng_state(),
             'rollout_cuda': self.rollout_sharding_manager.gen_random_states if self._is_rollout else None},
            os.path.join(state_dir, f'rng_rank{self.rank}.pt'),
        )
        if getattr(self.actor, 'moco_cagrad_tracker', None) is not None:
            from moco_checkpoint import save_worker_tracker
            save_worker_tracker(self, state_dir)
        torch.distributed.barrier()
        if self.rank == 0 and self.actor_lr_scheduler is not None:
            torch.save(self.actor_lr_scheduler.state_dict(), os.path.join(state_dir, 'scheduler.pt'))
            with open(os.path.join(state_dir, 'COMPLETE'), 'w', encoding='utf-8') as marker:
                marker.write('optimizer scheduler rng state complete\n')
        torch.distributed.barrier()
        if self._is_offload_param:
            offload_fsdp_param_and_grad(module=self.actor_module_fsdp, offload_grad=self._is_offload_grad)
