...
class Worker(Worker):
    ...
    def sync_model_weights(self, actor_weights: Dict, load_format: str):
        # CUDA graphs retain parameter addresses. In graph mode weights must
        # stay on the GPU and be updated in place by the standard loaders.
        graph_pointers = None
        if not self.model_config.enforce_eager:
            graph_pointers = tuple(p.data_ptr() for p in self.model_runner.model.parameters())
        ...
        if graph_pointers is not None:
            refreshed = tuple(p.data_ptr() for p in self.model_runner.model.parameters())
            if refreshed != graph_pointers:
                raise RuntimeError('Weight synchronization invalidated CUDA graph parameter addresses')

    def offload_model_weights(self) -> None:
        if not self.model_config.enforce_eager:
            # Offloading would leave captured graphs reading stale weights.
            # The rollout config also requires free_cache_engine=False.
            return
